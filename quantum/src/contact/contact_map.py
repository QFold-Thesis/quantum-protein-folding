...
class ContactMap:
    def __init__(self, protein: Protein):
        ...
        try:
            ...
            logger.debug(f"Contact map initialized successfully. Contacts detected: {self._contacts_detected}")
    ...
    def initialize_contact_map(self):
        ...
        for lower_bead_idx in range(0, main_chain_length - 2):
            for upper_bead_idx in range(lower_bead_idx + 2, main_chain_length):
                ...
                contact_operator: SparsePauliOp = self._create_main_main_contact(
                    upper_bead=upper_bead,
                    lower_bead=lower_bead
                )

                self.main_main_contacts[upper_bead.index][lower_bead.index] = contact_operator
                self._contacts_detected += 1



    def _create_main_main_contact(self, upper_bead: Bead, lower_bead: Bead) -> SparsePauliOp:
        """Creates a contact operator between two main chain beads."""

        z_op_index: int = (lower_bead.index) * (len(self._protein.main_chain) - 1) + (upper_bead.index)


        contact_operator = build_pauli_z_operator(
            num_qubits=self._num_contact_qubits,
            pauli_z_indices={z_op_index}
        )
